                ...
                for mes in data.get('messages'):
                    ...
                    if chat_number[1] == 'c.us':
                        ...
                        partners = request.env['res.partner'].sudo().search(
                            ['|', ('phone', 'ilike', number), ('mobile', 'ilike', number)])
                        ...
                        for partner in partners:
                            ...
                            if mes.get('self') == 0:
                                # phone change to mobile
                                # msg_exist = request.env['whatsapp.history'].sudo().search([('message_id','=', mes['id']),('partner_id','=', partner_id),('type','=','received')])
                                # if not msg_exist:
                                    if mes['type'] == 'chat':
                                        msg_exist = request.env['whatsapp.history'].sudo().search(
                                            [('message_id', '=', mes.get('id')), ('partner_id', '=', partner_id),
                                             ('type', '=', 'received')])
                                        if not msg_exist:
                                            vals = {
                                        ...
                                        msg_exist = request.env['whatsapp.history'].sudo().search(
                                            [('message_id', '=', mes['id']), ('partner_id', '=', partner_id),
                                             ('type', '=', 'received')])
                                        if not msg_exist:
